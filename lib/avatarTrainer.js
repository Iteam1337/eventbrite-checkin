...
const { map, filter, mergeMap, tap, mergeAll, catchError, doto, switchMap, finalize} = require('rxjs/operators')
...
const downloadAvatars = () => pipe(
  ...
  map(({email, id, github, facebook, twitter, linkedin, gravatar, manual}) => ({email, id, avatars: [github, facebook, twitter, gravatar, linkedin].filter(a => a)})),
...
function trainOnAttendeesFromEvent(eventId) {
  return new Promise(resolve => {
    from(getAttendees(eventId)).pipe(
      ...
      enrichProfile(),
      //tap(profile => console.log(`[x] Clearbit result ${profile.email}: ${Object.keys(profile).join(', ')}...`)),
      downloadAvatars(),
